...
| 短暂丢失后是否恢复同一实体和 ID |recognition_overlay.mp4 600-700fps  | 在短暂丢失后仍然为同一个目标 |通过 |
| 释放和重新选择是否合理 |recognition_overlay.mp4 200-300fps | 这期间多次有释放重新锁定新的实体，因为先前锁定实体完全消失 |通过 |
完整观看结论：【已完整播放 task_3 和 task_4 的标注视频，并对双目标片段及释放、恢复事件回看原视频。本次发现的主要问题为缺失帧数之后，锁定的目标id发生改变，但可能对应的是同一个目标，虽然对结果无大体影响，典型帧范围见上表。】。
...
ID 文本不变并不证明物理身份始终正确。
本机调整及效果：【本次使用当前提交代码中的参数，未额外调参。具体运行参数见本报告第 3 节；已知问题保留并在人工复查中记录。】。
